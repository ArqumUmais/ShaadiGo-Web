...
import { IconInquiry, IconAlert, IconFeedback, IconPhone, IconEmail, IconLocation, IconChat } from './Icons';

const INQUIRY_TYPES = [
  { key: 'inquiry',  Icon: IconInquiry,  name: 'General Inquiry',   desc: 'Questions about venues, availability, pricing, or how ShaadiGo works.' },
  { key: 'issue',    Icon: IconAlert,    name: 'Report an Issue',   desc: 'Booking disputes, payment problems, venue inaccuracies, or platform errors.' },
  { key: 'feedback', Icon: IconFeedback, name: 'Platform Feedback', desc: 'Share suggestions, improvements, or your overall experience with ShaadiGo.' },
];
...
export default function Contact() {
  ...
  const [form, setForm] = useState({ fname:'', lname:'', email:'', phone:'', subject:'', bookingRef:'', message:'' });
  ...
  const handleSubmit = async () => {
    ...
    if (Object.keys(newErrors).length) { setErrors(newErrors); setTimeout(() => setErrors({}), 2500); return; }
    try {
      await axios.post('http://localhost:5001/api/contact', {
        fname: form.fname, lname: form.lname, email: form.email, phone: form.phone,
        inquiryType: selectedType, subject: form.subject, bookingRef: form.bookingRef,
        priority, message: form.message,
      });
      setToast(true);
      setTimeout(() => setToast(false), 4000);
      setTimeout(() => { setForm({ fname:'', lname:'', email:'', phone:'', subject:'', bookingRef:'', message:'' }); setSelectedType('inquiry'); setPriority('med'); }, 500);
    } catch (err) {
    ...
    <div className="contact-page">
      ...
      <main className="contact-main">
        <div className="contact-page-title">
          ...
          <p>Submit a general inquiry, report an issue, or share your feedback.</p>
        </div>
        ...
        <div className="inquiry-types">
          {INQUIRY_TYPES.map(t => (
            <div key={t.key} className={`type-card ${selectedType === t.key ? 'selected' : ''}`} onClick={() => setSelectedType(t.key)}>
              ...
              <div className="type-icon-wrap"><t.Icon size={36} color={selectedType === t.key ? '#4D0D0D' : '#a0806a'} /></div>
        ...
        <div className="contact-layout">
          <div className="contact-form-card">
            ...
            <div className="contact-field-row">
              ...
              <div className="contact-field">
                <label className="contact-label">Booking Reference <span style={{opacity:0.45,fontWeight:400}}>(if applicable)</span></label>
                <div className="input-wrap">
            ...
            <div className="contact-field">
              ...
              <div className="priority-row">
                {[['low','Low'],['med','Medium'],['high','High / Urgent']].map(([p, label]) => (
                  <button key={p} className={`priority-btn p-${p} ${priority === p ? 'active' : ''}`} onClick={() => setPriority(p)}>
                    <svg width="10" height="10" viewBox="0 0 10 10"><circle cx="5" cy="5" r="4" fill={p==='low'?'#27ae60':p==='med'?'#f39c12':'#e74c3c'}/></svg>
                    {label}
                  </button>
                ))}
              </div>
            ...
            <div className="contact-field">
              ...
              <textarea className={`contact-textarea ${errors.message ? 'error' : ''}`}
            ...
            <div className="contact-form-note">
              <IconLockSmall /> Your information is private and never shared with third parties.
            </div>
          </div>
          ...
          <div className="contact-sidebar">
            <div className="sidebar-card">
              ...
              {[
                { Icon: IconPhone,    label: 'Phone',     value: '+92 21 3456 7890',      href: 'tel:+922134567890' },
                { Icon: IconEmail,    label: 'Email',     value: 'support@shaadigo.pk',   href: 'mailto:support@shaadigo.pk' },
                { Icon: IconChat,     label: 'Live Chat', value: 'Available 9 AM – 9 PM', href: '#' },
                { Icon: IconLocation, label: 'Office',    value: '12 MM Alam Rd, Lahore', href: '#' },
              ].map(c => (
                <a key={c.label} className="contact-channel" href={c.href}>
                  <div className="ch-icon-wrap"><c.Icon size={22} color="#4D0D0D" /></div>
                  <div>
...
function IconLockSmall() {
  return (
    <svg width="13" height="13" viewBox="0 0 16 16" fill="none" style={{marginRight:5,verticalAlign:'middle'}}>
